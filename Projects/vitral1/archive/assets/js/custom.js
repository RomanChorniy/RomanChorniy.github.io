...
;(function ($) {
    ...
    if (wrapper.length) {
      function setMarginTopWrapper() {
        if ($(window).width() > 576) {
          ...
          wrapper.css(
            'padding-top',
            heightTopHeader + heightNavbarWrapper + 'px'
          )
      ...
      function checkNotLoadedImg() {
        setMarginTopWrapper()
        var hasLoaded = $('.top-header', wrapper)
          .find('img.lazy')
          .hasClass('loaded')
  ...
  function sliderSlickInit() {
    var gallery = $(
      '.slider-wrapper:not(.hide-slider, .image-desktop, .image-mobile)'
    )
    if (gallery.length) {
    ...
    if (signatureImageWrap.length) {
      signatureImageWrap.each(function () {
        ...
        $('.component-button', thisSignatureImage).on('click', function () {
          ...
          var countViewWhenClickOnButton = thisSignatureImageButton.data(
            'count-view-when-click-on-button'
          )
          if (countViewWhenClickOnButton) {
            $(thisSignatureImage)
              .find('.hide-block')
              .slice(0, countViewWhenClickOnButton)
...
jQuery(document).ready(function () {
  ...
  jQuery('.title__slider').slick({
    ...
    prevArrow:
      ...
      templateUrl +
      '/assets/images/Arrow-down.svg" alt="prevArrow"></div>',
    nextArrow:
      '<div class="slick-next"><img src="' +
      templateUrl +
      '/assets/images/Arrow-down.svg" alt="nextArrow"></div>',
    responsive: [
      {
        breakpoint: 575,
        settings: {
  ...
  jQuery('.title__slider').on('afterChange', function () {
    ...
    if (currentSlide == 0) {
      jQuery('.title__slider .slick-prev').hide()
      jQuery('.title__slider .slick-next').show()
    } else if (currentSlide == slides - 1) {
      jQuery('.title__slider .slick-next').hide()
      jQuery('.title__slider .slick-prev').show()
    }
  ...
  jQuery('.partners__slider').slick({
    ...
    prevArrow:
      '<div class="slick-prev"><img src="' +
      templateUrl +
      '/assets/images/Arrow-prev.svg" alt="prevArrow"></div>',
    nextArrow:
      '<div class="slick-next"><img src="' +
      templateUrl +
      '/assets/images/Arrow-next.svg" alt="nextArrow"></div>',
    responsive: [
      {
        breakpoint: 600,
        settings: {
  ...
  jQuery('.aboutGoods .seeMore').on('click', function () {
    ...
    if (jQuery('.article__text_hideText').hasClass('visible'))
      jQuery('.aboutGoods .seeMore').text('Свернуть')
  ...
  jQuery('.blockBtn').on('click', function () {
    ...
    if (jQuery('.colorRal').hasClass('hideBlock'))
      jQuery('.blockBtn .siteBtn').text('Скрыть цвета')
